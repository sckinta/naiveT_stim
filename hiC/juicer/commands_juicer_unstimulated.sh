...
condition="naiveT_unstimulated_2reps" # change
dir="$parent_dir/$condition"
hicup_parent_dir="/mnt/isilon/sfgi/suc1/analyses/wells/hiC/hicup" # change
hicup_subdirs=("naiveT_unstimulated_ND517" "naiveT_unstimulated_TMP442") # change
cd $dir
...
" > $dir/scripts/cooler_merge.$resol.sh
        cd $dir/scripts
        qsub -cwd cooler_merge.$resol.sh
done
...
resolutions=("1K" "1500" "2000" "2500" "4000" "5K")
resolutions=("500")
...
################################## fithic_loops #################################
cd $dir/cool/merge/
ln -s $condition.1K.cool $condition.1000.cool
...
resols=("1000" "2000" "4000")
for resol in ${resols[@]}; do
        ls $dir/fithic_loops/$resol/*/FitHiC.spline_pass2.res*.significances.txt.gz | wc -l
done
...
# 100785 1000/ACT_adjusted.res1000.sigInt.bedpe
# 115863 1000/FitHiC.spline_pass2.res1000.sigInt.bedpe
# 498925 2000/ACT_adjusted.res2000.sigInt.bedpe
# 633596 2000/FitHiC.spline_pass2.res2000.sigInt.bedpe
# 1318473 4000/ACT_adjusted.res4000.sigInt.bedpe
# 1768316 4000/FitHiC.spline_pass2.res4000.sigInt.bedpe
...
" > $dir/hic_loops/scripts/$prefix.sh
        ...
        sbatch --mem 16G $dir/hic_loops/scripts/$prefix.sh
done

### put summary together
cd $dir/hic_loops
files=($(ls $dir/hic_loops/anno/*.anno.summary.txt)) # 15
for i in `seq 1 ${#files[@]}`; do
        j=$((i-1))
        if [[ "$i" -eq "1" ]]; then
                cat ${files[$j]}
