# v0.7.4 split
PROMOTEROME_PARENT_DIR="/mnt/isilon/sfgi/suc1/analyses/wells/hiC/hicup"

REP="ND517" # change
PROMOTEROME_DIR="naiveT_8hr_"$REP # change
FASTQ_R1="naiveT_8hr_"$REP"_R1.fastq.gz" # change
FASTQ_R2="naiveT_8hr_"$REP"_R2.fastq.gz" # change
RAWDATA_DIR="/mnt/isilon/sfgi/rawData/wells/hiC/PMBC_naiveT_stim/naiveT_8hr/naiveT_8hr_"$REP  # change
...
qsub -cwd -j y -o split_1_1.log -l h_vmem=16G ~/captureC/scripts/bash/splitFastqGz.sh ../$FASTQ_R1 200000000 1 R1
qsub -cwd -j y -o split_1_2.log -l h_vmem=16G ~/captureC/scripts/bash/splitFastqGz.sh ../$FASTQ_R2 200000000 1 R2
...
-g "hg19" \
-t 8 -m "4G" -s 1 \
-b 2.5M,1M,500K,250K,100K,50K,25K,10K,5K,1K,500,1500,2000,2500,4000,40K \
$PAIR_IN \
$COOL_OUT_PREFIX
...
-g "hg19" -t 4 -m "120G" -s 1 \
-b 2.5M,1M,500K,250K,100K,50K,25K,10K,5K,1K,500,1500,2000,2500,4000,40K \
$PRE_IN $HIC_OUT


####################### summarise naiveT all hicup #####################
cd $PROMOTEROME_PARENT_DIR

files=($(ls naiveT*/hicup/*_ShortSummary.txt))
for file in ${files[@]}; do
        sed 1d $file
done > naiveT_hicup_ShortSummary.txt
cat <(head -n 1 ${file[0]}) naiveT_hicup_ShortSummary.txt > naiveT_hicup_ShortSummary2.txt
mv naiveT_hicup_ShortSummary2.txt naiveT_hicup_ShortSummary.txt
